...
function preparedGameHtml() {
  ...
  const critical = `
<style id="h38-critical">
html,body{margin:0;min-height:100%;background:#171310;color:#f4eadb;font-family:system-ui,-apple-system,sans-serif}
#h38boot{position:fixed;left:10px;right:10px;top:max(10px,env(safe-area-inset-top));z-index:999999;background:#211b17f5;color:#fff;border:1px solid #8b7659;border-radius:10px;padding:10px 12px;font:12px/1.35 ui-monospace,Menlo,monospace;box-shadow:0 8px 24px #0008;pointer-events:none}
#h38boot strong{color:#e6c27d}#h38boot .ok{color:#8fd19e}#h38boot .bad{color:#ff9c8f}
</style>`;

  const loader = `
<div id="h38boot"><strong>MTG TableTop H38</strong><div id="h38line">Preparing game…</div></div>
<script>
(()=>{
  const box=document.getElementById('h38boot'), line=document.getElementById('h38line');
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/html-test') {
      return new Response(
        '<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><body style="font-family:system-ui;background:#181410;color:#fff;padding:24px"><h1>HTML delivery works</h1><p>Build H38</p></body>',
