...
function parseText(text) {
  ...
  for (const rawLine of String(text || '').split(/\r?\n/)) {
    ...
    if (/^commander/.test(heading)) {
      zone = 'commander';
      continue;
    }
    if (/^sideboard/.test(heading)) {
      zone = 'sideboard';
      continue;
    }
    if (/^tokens?/.test(heading)) {
      zone = 'tokens';
      continue;
    }
  ...
  return json(
    {
      ...
      detail:
        'Make sure the deck is Public. If URL import is blocked by Moxfield, switch to Card List and paste the exported deck list.',
      attempts,
    },
    502,
  );
