...
function preparedGameHtml() {
  ...
  const critical = `
<style id="h39-critical">
html,body{margin:0;min-height:100%;background:#171310;color:#f4eadb;font-family:system-ui,-apple-system,sans-serif}
#h39boot{position:fixed;left:10px;right:10px;top:max(10px,env(safe-area-inset-top));z-index:999999;background:#211b17f5;color:#fff;border:1px solid #8b7659;border-radius:10px;padding:10px 12px;font:12px/1.35 ui-monospace,Menlo,monospace;box-shadow:0 8px 24px #0008;pointer-events:none}
#h39boot strong{color:#e6c27d}#h39boot .ok{color:#8fd19e}#h39boot .bad{color:#ff9c8f}
</style>`;

  const loader = `
<div id="h39boot"><strong>MTG TableTop H39</strong><div id="h39line">Preparing game…</div></div>
<script>
(()=>{
  const box=document.getElementById('h39boot'), line=document.getElementById('h39line');
...
export default {
  async fetch(request, env, ctx) {
    ...
    if (url.pathname === '/api/html-test') {
      return new Response(
        '<!doctype html><meta name="viewport" content="width=device-width,initial-scale=1"><body style="font-family:system-ui;background:#181410;color:#fff;padding:24px"><h1>HTML delivery works</h1><p>Build H39</p></body>',
